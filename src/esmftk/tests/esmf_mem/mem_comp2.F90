...
module NUOPC_COMP2
  ...
  !-----------------------------------------------------------------------------
  ! NUOPC_COMP2: Generic Component 2 (basic ocean)
  !-----------------------------------------------------------------------------
  ...
  subroutine Advertise(model, rc)
    ...
    write(*,*) "Comp2 testType=",trim(testType)
  ...
  subroutine Realize(model, rc)
    ...
    if ((testType == "geom") .or. &
       ...
       case ('default') ! By default, just build a small Grid       
          ...
          defaultGrid = ESMF_GridCreateNoPeriDimUfrm(maxIndex=(/15, 16/), &
  ...
  subroutine Advance(model, rc)
    ...
    call ESMF_ClockPrint(clock, options="currTime", &
      preString="------>Advancing COMP2 from: ", unit=msgString, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return
